...
Client.prototype.requestUpload = function (uri, stream, callback) {
  var self     = this,
      out,
      auth     = this.options.get('password') ? ':' + this.options.get('password') : '/token:' + this.options.get('token'),
      encoded  = new Buffer(this.options.get('username') + auth).toString('base64'),
  ...
  /*
   * For some reason the body is making its way here from my request to upload
   * a CSV in node-sebastian. This means the body of the ticket from some
   * other customer will end up at the end of this CSV file, and I have
   * no idea how. Delete the body explicitely here.
   */
